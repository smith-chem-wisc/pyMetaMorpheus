...
class TaskResult:
    ...
    def summary(self) -> Path | None:
        """``results.txt`` — MetaMorpheus's human-readable run summary (ID counts,
        settings). The best first thing to read after a run."""
        return self.file("results.txt")

    @property
    def prose(self) -> Path | None:
        """``AutoGeneratedManuscriptProse.txt`` — a methods paragraph describing
        exactly how this task ran, ready to drop into a paper."""
        return self.file("AutoGeneratedManuscriptProse.txt")
